...
void BPlusTreePage::IncreaseSize(int amount) {
  size_ += amount;
  BUSTUB_ASSERT(size_ <= max_size_ + 1, "Number of Key & Value pairs in page large max number + 1.");
}
void BPlusTreePage::DecreaseSize(int amount) {
  size_ -= amount;
  BUSTUB_ASSERT(size_ >= 0, "Number of Key & Value pairs in page less 0.");
}
 ...
 */
auto BPlusTreePage::GetMinSize() const -> int { return (max_size_ & 1) == 1 ? (max_size_ + 1) / 2 : max_size_ / 2; }
