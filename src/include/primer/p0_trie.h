...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    ...
    bool ok = children_.insert(std::make_pair(key_char, std::move(child))).second;
    return ok ? &(children_.at(key_char)) : nullptr;
  }
...
class Trie {
 private:
  ...
  /* Insert */
  template <typename T>
  bool Insert(std::unique_ptr<TrieNode> *root, const std::string &key, std::size_t index, T value) {
    if (index == key.length()) {
      if ((*root)->IsEndNode()) {
        return false;
      }
      std::unique_ptr<TrieNode> rootp = std::make_unique<TrieNodeWithValue<T>>(std::move(**root), value);
      root->swap(rootp);
      return true;
    }

    char key_char = key[index];
    if (!(*root)->HasChild(key_char)) {
      (*root)->InsertChildNode(key_char, std::make_unique<TrieNode>(key_char));
    }
    return Insert((*root)->GetChildNode(key_char), key, index + 1, value);
  }
  /* Remove */
  bool Remove(std::unique_ptr<TrieNode> *root, const std::string &key, std::size_t index, bool &code) {
    if (index == key.length()) {
      code = (*root)->IsEndNode();
    ...
    char key_char = key[index];
    if (!(*root)->HasChild(key_char)) {
      return (*root)->IsEndNode() || (*root)->HasChildren();
    }
    bool has_children = Remove((*root)->GetChildNode(key_char), key, index + 1, code);
    if (!has_children) {
      (*root)->RemoveChildNode(key_char);
    }
    return (*root)->IsEndNode() || (*root)->HasChildren();
  }
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    latch_.WLock();
    bool result = Insert(&root_, key, 0, value);
    latch_.WUnlock();
    return result;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    latch_.RLock();
    std::unique_ptr<TrieNode> *root = &root_;
    for (char key_char : key) {
      root = (*root)->GetChildNode(key_char);
      if (root == nullptr) {
        latch_.RUnlock();
        return {};
      }
    ...
    auto tp = dynamic_cast<TrieNodeWithValue<T> *>((*root).get());
    if (tp == nullptr) {
      latch_.RUnlock();
      return {};
    }
    *success = true;
    T result = tp->GetValue();
    latch_.RUnlock();
    return result;
  }
};
